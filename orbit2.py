...
for file in files:
    if "なし" in file:
        visualize_color = (255, 0, 0)#線の色
    else:
        visualize_color = (0, 0, 255)#線の色
    ...
    writer = cv2.VideoWriter(f_output + file[:-4] + "_dst.mp4", fourcc, fps, (width, height))#まとめて変換
    traj = np.zeros((height, width), np.uint8)  # 手の軌跡の画像
    cpos_history = []  # フレームiのレーザポインタ中心の座標

    #retはcap.readに画像情報が格納されているかの確認
    # 初期の点を探す
    ret, init = cap.read()
    r0 = init[:, :, 2]
    cap.set(cv2.CAP_PROP_POS_FRAMES, 100)#100フレーム後から読み込み
    ret, tmp = cap.read()
    r1 = tmp[:, :, 2]
